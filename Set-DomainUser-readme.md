...
| PSMConnectCredentials           | PSMConnect user credentials.                                              | Set-DomainUser will attempt to retrieve this information from the Vault. If not present, you will be prompted to provide them.            |
| PSMAdminConnectCredentials      | PSMAdminConnect credentials.                                              | Set-DomainUser will attempt to retrieve this information from the Vault. If not present, you will be prompted to provide them.            |
...
| IgnoreShadowPermissionErrors    | Ignore shadow user permission errors.                                     | Live session monitoring may not function until permissions are adjusted.                                                                  |
| SkipPSMUserTests                | Do not check PSM users for configuration errors.                          | Review user configuration against requirements manually after running.                                                                    |
| SkipPSMObjectUpdate             | Do not configure the PSM server object with the updated PSM user details. | Edit Configuration Options in Privilege Cloud manually to provide PSM user object information.                                            |
| SkipSecurityPolicyConfiguration | Do not update Local Security Policy                                       | Security policy must be configured manually (or via domain policies) to allow PSM users to log on with Remote Desktop.                    |
| SkipAddingUsersToRduGroup       | Do not add PSM users to the Remote Desktop Users group.                   | Perform this step manually after running.                                                                                                 |
